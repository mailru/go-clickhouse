...
func TestParseDSN(t *testing.T) {
	dsn := "http://username:password@localhost:8123/test?timeout=1s&idle_timeout=2s&read_timeout=3s" +
		"&write_timeout=4s&location=Local&max_execution_time=10&debug=1&buff_size=100"
	...
	if assert.NoError(t, err) {
		...
		assert.EqualValues(t, 100, cfg.BuffSize)
	}
}
...
func TestDefaultConfig(t *testing.T) {
	...
	assert.EqualValues(t, defaultBuffSize, cfg.BuffSize)
...
func TestFormatDSN(t *testing.T) {
	dsn := "http://username:password@localhost:8123/test?timeout=1s&idle_timeout=2s&read_timeout=3s" +
		"&write_timeout=4s&location=Europe%2FMoscow&max_execution_time=10&debug=1&buff_size=100"
	cfg, err := ParseDSN(dsn)
	if assert.NoError(t, err) {
		dsn2 := cfg.FormatDSN()
		...
		assert.Contains(t, dsn2, "buff_size=100")
	}
}
